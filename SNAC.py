...
#------------Tela de Lobby / Segunda tela------------
    ...
    def janela_lobby(self):
        ...
        if self.recebe_usuario == '' or self.recebe_senha == '':
            ...
            try:
                if self.recebe_usuario and self.recebe_senha in self.pesquisa_dados:
    ...
    def cadastrar (self):
        ...
        try:
            if self.capturar_nome == '' or self.capturar_email == '' or self.capturar_senha == '' or self.capturar_confirmar_senha == '':
                messagebox.showwarning('Sistema de cadastro', 'Preencha todos os campos!')
            elif len(self.capturar_nome)<3:
                messagebox.showwarning('Sistema de cadastro', 'O nome não pode ser menor que 3 caracteres')
            elif len(self.capturar_email)<10:
                messagebox.showwarning('Sistema de cadastro', 'Email inválido')
            elif len(self.capturar_senha)<8 or len(self.capturar_senha)>=16:
                messagebox.showwarning('Sistema cadastro', 'Tamanho da senha inválido')
            elif self.capturar_senha != self.capturar_confirmar_senha:
                messagebox.showwarning('ERRO', 'Senha não coincidem')
            else:
                self.conexao.commit()
...
# -------------------------------------ALERTA----------------------------
    def alerta(self):
        messagebox.showwarning('Aviso', 'Função indisponivel')
